/*
 * compute.ts — the engine's single compute dispatcher (runCompute), extracted
 * from cli.ts so the browser bundle can import it without pulling in the CLI's
 * node:fs stdin handling. Node entry points (cli.ts, index.ts, mcp-server) are
 * unchanged consumers; in the web build the ./memory.js import below is swapped
 * for a stub (scripts/build-web.mjs) — journaling is a Node-only feature.
 */
